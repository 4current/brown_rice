...
combined.df <- full_join(white.nutrients.df, brown.nutrients.df, by="nutrient", suffix=c(".white",".brown")) %>%
  filter(unit.brown=="G" | unit.white == "G" | unit.brown == "KCAL" | unit.white == "KCAL") %>%
...
kable(combined.df, format = "latex", booktabs = TRUE) 
...
bp <- barplot(height=as.matrix(compared.df[,3:6]), main="Nutrients", legend.text = shortNames, beside = TRUE)
print(bp)
